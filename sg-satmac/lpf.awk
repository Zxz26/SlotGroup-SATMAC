...
#	printf "%f, %d, %d\n",total_channel_utilization,node_count,now;
	now = now/1000;
printf "%d %d\n" ,now,read;
	printf "avg_wait %.4f avg_no_valid %.1f avg_col %.1f avg_col_trace %.1f avg_req_fail %.1f avg_adj_req %.1f avg_adj_suc %.1f avg_rx: %.1f avg_tx: %.1f frame_len %.1f ch_util %.3f\n", total_wait/total_frame, total_no_valid/node_count, total_collision/node_count, 0, total_req_fail/node_count, total_adj_req/node_count, total_adj_suc/node_count, total_rx/now/node_count, total_tx/now/node_count, total_frame_len/now/node_count, total_channel_utilization/now/node_count;
}
