...
MacLow::ReceiveOk (Ptr<Packet> packet, double rxSnr, WifiTxVector txVector, bool ampduSubframe)
{
  ...
  if (hdr.IsRts ())
  ...
  else if (hdr.IsCtl () && !(hdr.IsSatmacData()))
    {
      if (hdr.IsCfEnd ())
        {
  ...
  else if (hdr.GetAddr1 ().IsGroup ())
    {
      if (ampduSubframe)
        ...
        {
          if (hdr.IsData () || hdr.IsMgt () || hdr.IsSatmacData() )
            {
...
Time
MacLow::GetLastPktTxDuration (void) const
{
	return m_lastpkt_txDuration;
}
...
MacLow::StartDataTxTimers (WifiTxVector dataTxVector)
{
  ...
  m_lastpkt_txDuration = txDuration;
